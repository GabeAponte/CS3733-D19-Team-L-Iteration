...
public class PrescriptionRequestAccess extends DBAccess{

    private static final DateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
    private static final DateFormat tdf = new SimpleDateFormat("HHmm");

    /**@author Alex
     * deletes all the records from the prescrptionRequest table
     */
    ...
    /**@author Alex
     * adds a new presciption request to the database
     *
     */
    public void makeRequest(String desc, String medtype, String destination, String deliveryTime, String medamount){
        String sql = "insert into prescriptionRequest(" +
                "comment, medtype, destination, deliveryTime, medamount)" +
                "values (?, ?, ?, ?, ?)";
        ...
        try (Connection conn = this.connect();
            ...
            pstmt.setString(2, medtype);
            pstmt.setString(3, destination);
            pstmt.setString(4, deliveryTime);
            pstmt.setInt(5, Integer.parseInt(medamount));
            pstmt.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
