...
public class RoomAccess extends DBAccess {
    ...
    public ArrayList<String> getAvailRooms(String date, int startTime, int endTime){
        ...
        String sql = "select roomID, name from room left outer join (select rID from reservation where rdate = ? and startTime between ? and ? or endTime between ? and ?) on roomID = rID where rID is null;";
        ArrayList<String> data = new ArrayList<String>();
        try (Connection conn = this.connect();
