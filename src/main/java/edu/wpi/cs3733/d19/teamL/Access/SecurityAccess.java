...
public class SecurityAccess extends DBAccess{
    ...
    public void deleteRecords() {
        String sql = "Delete from securityRequest;";
...
*/
    ...
    public TreeItem<ServiceRequestTable> getSecurityRequests(int getNum){
        ...
        String sql = "SELECT * FROM securityRequest where requestID is not NULL and fulfilled = 0";
        int count = 0;
    ...
    public int countRecords() {
        String sql = "select COUNT(*) from securityRequest where requestID is not null and fulfilled = false";
        try (Connection conn = this.connect();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
